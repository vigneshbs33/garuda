...
This document outlines the slide-by-slide presentation structure for the **GARUDA** project. Each slide includes the **Slide Visuals** (what to show) and the **Speaker Notes** (what to say).
...
## 🗂️ Slide 7: Technical Stack
*   **Slide Title:** The Production-Ready Tech Stack
*   **Visuals:** Grouped technology boxes.
*   **Key Bullets:**
    *   **Computer Vision & ML:** PyTorch, YOLOv8/v11 (Detection), ByteTrack (Multi-object tracking), MediaPipe FaceMesh (Drowsiness), PaddleOCR (Plate reading).
    *   **Backend:** FastAPI (asynchronous, high-concurrency REST & WebSocket server), SQLAlchemy 2.0 (async ORM), SQLite/PostgreSQL.
    *   **Frontend Dashboard:** Next.js, React, TypeScript, Leaflet.js (Geospatial heatmaps).
    *   **Alerting & Training:** Twilio SMS API, Flower (Federated Learning framework).
*   **Speaker Notes:**
    > "Our stack is built for speed and modularity. On the ML side, we run YOLOv8 and YOLO11 alongside ByteTrack for tracking. The backend is written in FastAPI with async database handling, capable of processing hundreds of edge events simultaneously. The live control dashboard is a modern Next.js and React app that provides real-time maps, visual analytics, and the review interface for officers."

---

## 🗂️ Slide 8: Future Roadmap & Under Development
*   **Slide Title:** Current Status & Future Vision
*   **Visuals:** Timeline chart showing Phase 1 (Core ML - Done), Phase 2 (Edge Porting - Under Development), Phase 3 (Cross-Camera Tracking - Future).
*   **Key Bullets:**
    *   **Under Development (Active Phase):**
        *   TFLite INT8 quantization benchmarks on physical Raspberry Pi 5.
        *   Full database candidate search matching color/type against the registry.
        *   Deploying real Twilio SMS gateway from mock mode.
    *   **Future Scope:**
        *   *Cross-Camera Re-ID:* Tracking vehicles across different intersection cameras using OSNet (without plate reliance).
        *   *Federated Loop:* Implementing online backpropagation on the edge nodes using verified officer inputs.
*   **Speaker Notes:**
    > "Where are we now? The core ML pipeline, Next.js interface, and async backend are complete. We are currently benchmarking our quantized models on physical Raspberry Pi 5 hardware and refining our database candidate-search algorithm. Our future roadmap includes cross-camera vehicle re-identification, letting us track a vehicle across multiple intersections even if they don't have readable plates, and finalizing the federated learning backpropagation loops on the edge."
